...
class ConditionalDecodeHead(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.guidance_project = self.build_project(
            channels,
            num_classes,
            groups=1,
            num_convs=1,
            conv_cfg=conv_cfg)
        self.filter_project = self.build_project(
    ...
    def cls_seg(self, feat):
        ...
        cond_filters = (
            (value.shape[2] * value.shape[3])**-1) * cond_filters
    ...
    def losses(self, logits, seg_label):
        ...
        cond_label = resize(
            input=seg_label, size=cond_logit.shape[2:], mode='nearest')
